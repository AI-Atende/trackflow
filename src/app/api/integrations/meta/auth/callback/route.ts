...
export async function GET(request: Request) {
  ...
  if (!META_APP_ID || !META_APP_SECRET) {
    console.error('Meta Auth Callback Error: Missing App ID or Secret', {
      hasAppId: !!META_APP_ID,
      hasSecret: !!META_APP_SECRET,
    });
    return new NextResponse('Server Configuration Error: Meta App ID or Secret is missing.', {
      status: 500,
    });
  }
  ...
  try {
    ...
    if (!appUrl) {
      ...
      return new NextResponse('Server Configuration Error: NEXT_PUBLIC_APP_URL is missing.', {
        status: 500,
      });
    }
