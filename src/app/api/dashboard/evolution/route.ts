...
export async function GET(request: Request) {
  ...
  try {
    ...
    const kommoDailyMap: Record<string, { stages: Record<string, number>; revenue: number }> = {};
      ...
      if (config && subdomain) {
        ...
        const fetchDay = async (date: Date) => {
          ...
          try {
            ...
            const isAuthError = e instanceof Error && e.message?.includes('KOMMO_AUTH_ERROR');
            if (isAuthError) {
    ...
    const evolutionData = dates.map((d) => {
      ...
      const hasKommo =
        (dataSource.includes('KOMMO') || dataSource.includes('HYBRID')) &&
        activeJourneyMap.length > 0;
      const finalRevenue = hasKommo ? dayKommo.revenue : 0;
      const roas = totalSpend > 0 ? finalRevenue / totalSpend : 0;

      // Base object (plus one dynamic numeric field per journey stage, added below)
      const result: {
        date: string;
        revenue: number;
        receive: number;
