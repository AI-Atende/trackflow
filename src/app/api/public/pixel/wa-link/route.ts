...
export async function POST(req: NextRequest) {
  ...
  if (!linkId || !sessionCode) {
    return withPixelCors(
      NextResponse.json({ error: 'linkId and sessionCode are required' }, { status: 400 }),
    );
  }

  const link = await prisma.trackingLink.findUnique({ where: { id: linkId } });
  if (!link) {
    return withPixelCors(NextResponse.json({ error: 'Unknown tracking link' }, { status: 404 }));
  }

  const session = await prisma.pixelSession.findUnique({ where: { sessionCode } });
  if (!session || session.clientId !== link.clientId) {
    return withPixelCors(
      NextResponse.json({ error: 'Unknown or mismatched session' }, { status: 404 }),
    );
  }
  ...
  return withPixelCors(NextResponse.json({ waLink }));
}
