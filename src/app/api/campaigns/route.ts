...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (source === 'KOMMO' || source === 'HYBRID') {
      ...
      const { subdomain } = (config?.config as { subdomain?: string } | null) || {};
    ...
    if (source === 'META' || source === 'HYBRID' || source === 'KOMMO') {
      if (session.user.metaAdAccount?.adAccountId) {
        try {
          ...
          const metaData = await fetchMetaHierarchy(
            session.user.metaAdAccount.adAccountId,
            metaSince,
            metaUntil,
          );
          ...
          if (source === 'HYBRID' || source === 'KOMMO') {
            ...
            const findMatches = (
              nodes: CampaignHierarchy[],
              target: CampaignHierarchy,
              usedIds: Set<string>,
            ) => {
              ...
              if (matches.length === 0) {
                matches = nodes.filter((n) => {
                  ...
                  return (
                    nNameClean.includes(targetNameClean) || targetNameClean.includes(nNameClean)
                  );
                });
              }
            ...
            campaigns.forEach((kCamp) => {
              ...
              if (matches.length > 0) {
                ...
                kCamp.metaLeads =
                  (kCamp.metaLeads || 0) + matches.reduce((s, m) => s + (m.metaLeads || 0), 0);
                ...
                if (allMetaAdSets.length > 0) {
                  ...
                  kCamp.children.forEach((kAdSet) => {
                    ...
                    if (adSetMatches.length > 0) {
                      ...
                      kAdSet.spend =
                        (kAdSet.spend || 0) + adSetMatches.reduce((s, m) => s + m.spend, 0);
                      kAdSet.metaLeads =
                        (kAdSet.metaLeads || 0) +
                        adSetMatches.reduce((s, m) => s + (m.metaLeads || 0), 0);
                      ...
                      if (allMetaAds.length > 0) {
                        ...
                        kAdSet.children.forEach((kAd) => {
                          ...
                          if (adMatches.length > 0) {
                            ...
                            kAd.spend =
                              (kAd.spend || 0) + adMatches.reduce((s, m) => s + m.spend, 0);
                            kAd.metaLeads =
                              (kAd.metaLeads || 0) +
                              adMatches.reduce((s, m) => s + (m.metaLeads || 0), 0);
                          }
                        });

                        // Append Orphan Meta Ads
                        const orphanMetaAds = allMetaAds
                          .filter((m) => !usedMetaAdIds.has(m.id))
                          .map((m) => ({
                            ...m,
                            isOrphan: true,
                            data: {
            ...
            if (source === 'HYBRID') {
              const orphanMetaCampaigns = metaData
                .filter((m) => !usedMetaIds.has(m.id))
                .map((m) => ({ ...m, isOrphan: true }));
              // Append orphans
