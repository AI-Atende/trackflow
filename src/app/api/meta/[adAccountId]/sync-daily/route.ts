...
export async function POST(
  ...
  if (!metaAccount) {
    console.error(
      `[SYNC] Account not found for user ${session.user.clientId} and adAccount ${adAccountId}`,
    );
    return NextResponse.json({ error: 'Conta de anúncios não encontrada.' }, { status: 404 });
  }

  console.log(
    `[SYNC] Found account: ${metaAccount.id}, Status: ${metaAccount.status}, Name: ${metaAccount.name}`,
  );
  ...
  if (!hasAccess) {
    return NextResponse.json({ error: 'Acesso negado a esta conta de anúncios.' }, { status: 403 });
  }

  if (metaAccount.status.toLowerCase() !== 'active') {
    console.error(`[SYNC] Account status is ${metaAccount.status}, expected active`);
    return NextResponse.json({ error: 'A conta de anúncios não está ativa.' }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: 'Erro ao sincronizar insights', details }, { status: 500 });
  }
}
