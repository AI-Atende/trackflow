...
import { LayoutDashboard, BarChart3, TrendingUp, RefreshCw, Layers, Filter } from 'lucide-react';
import { Account, CampaignHierarchy, EvolutionDataPoint, MetricSummary } from '@/types';
...
const HomeContent = () => {
  ...
  const [evolutionData, setEvolutionData] = useState<EvolutionDataPoint[]>([]); // New State for Evolution Data
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/dashboard/data?since=${since}&until=${until}&dataSource=${dataSource}`,
      );
      if (res.ok) {
        const data = await res.json();
  ...
  }, [selectedAccount, dateRange, dataSource]);

  useEffect(() => {
    if (status === 'authenticated') {
      (async () => {
        await fetchAvailableDataSources();
      })();
    }
  }, [status, fetchAvailableDataSources]);
        ...
        .then((data) => {
          if (Array.isArray(data)) {
            ...
            setSelectedAccount((prev) => {
              if (!prev || !data.find((a) => a.id === prev.id)) {
                const ownAccount = data.find((a) => a.id === session.user.clientId);
                return ownAccount || data[0];
              }
              return prev;
            });
          }
  ...
  const metrics = React.useMemo(() => {
    ...
    const totalSpend = targetData.reduce((acc, c) => acc + (c.spend || 0), 0);
    const totalRevenue = targetData.reduce((acc, c) => acc + (c.revenue || 0), 0);
    const totalROAS = totalSpend > 0 ? totalRevenue / totalSpend : 0;

    const stageTotals = [
      targetData.reduce((acc, c) => acc + (c.data?.stage1 || 0), 0),
      targetData.reduce((acc, c) => acc + (c.data?.stage2 || 0), 0),
      targetData.reduce((acc, c) => acc + (c.data?.stage3 || 0), 0),
      targetData.reduce((acc, c) => acc + (c.data?.stage4 || 0), 0),
      targetData.reduce((acc, c) => acc + (c.data?.stage5 || 0), 0),
    ];
  ...
  const displayCampaignName = React.useMemo(() => {
    ...
    const findInCampaigns = (camps: CampaignHierarchy[]): string | null => {
      ...
      <Sidebar
        ...
        availableAccounts={
          availableAccounts.length > 0
            ? availableAccounts
            : [{ id: session.user.clientId, name: session.user.name, image: session.user.image }]
        }
        onAccountChange={(id) => {
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 bg-background">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-3">
                  ...
                  <h2
                    className="text-xl font-bold text-foreground tracking-tight truncate max-w-[200px] md:max-w-[400px]"
                ...
                <p className="text-sm text-muted-foreground">
                  Visão estratégica e saúde do negócio.
                </p>
              </div>
              ...
              <div className="w-48">
                <Select
                  options={availableDataSources.map((ds) => {
                    ...
                    } else if (ds === 'HYBRID_META') {
                      label = 'Kommo + Meta';
                      icon = <TrendingUp size={16} />;
                    } else if (ds === 'HYBRID_GOOGLE') {
                      label = 'Kommo + Google';
                      icon = <TrendingUp size={16} />;
                    } else if (ds === 'HYBRID_ALL') {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {isLoadingData
                    ? [...Array(3)].map((_, i) => (
                        <MetricCard
                          key={i}
                          metric={{ label: '', value: '', percentage: '', trend: 'neutral' }}
                          loading={true}
                        />
                      ))
                  ...
                  {metrics.length === 0 && !isLoadingData && (
                    <div className="col-span-3 text-center py-8 text-muted-foreground">
                      Nenhuma métrica disponível. Vincule uma conta de anúncios e sincronize os
                      dados.
                    </div>
                  )}
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <section className="w-full h-[400px]">
              <EvolutionChart data={evolutionData} loading={isLoadingData} />
            </section>
