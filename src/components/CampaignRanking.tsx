...
export const CampaignRanking: React.FC<CampaignRankingProps> = ({
  campaigns,
  loading,
  journeyLabels = [],
}) => {
  ...
  if (loading) {
    return (
      <div className="h-full w-full bg-card rounded-2xl p-6 border border-border animate-pulse" />
    );
  }
  ...
  const topCampaigns = [...campaigns]
    ...
    .sort((a, b) => {
      ...
      const getTicketMedio = (camp: AdCampaign | CampaignHierarchy) => {
        const lastIdx = journeyLabels.length || 5;
        const stageData = camp.data as Record<string, number> | undefined;
        const sales = stageData?.[`stage${lastIdx}`] || 0;
      ...
      const getConvRate = (camp: AdCampaign | CampaignHierarchy, stageIndex: number) => {
        const stageData = camp.data as Record<string, number> | undefined;
        const stageVal = stageData?.[`stage${stageIndex}`] || 0;
        const leads = stageData?.stage1 || 1;
  ...
  const data = (currentCamp.data as Record<string, number> | undefined) || { stage1: 0 };
  ...
  const salesCount = data[`stage${lastStageIdx}`] || 0;
  ...
  return (
    <div className="h-full w-full bg-card rounded-2xl p-4 border border-border shadow-sm glass flex flex-col relative overflow-hidden group">
      ...
      {/* Carousel Item */}
      <div
        className={`flex-1 flex flex-col rounded-xl bg-gradient-to-br ${currentStyle} border p-4 relative overflow-hidden transition-all duration-500`}
      >
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <div className="mb-4">
            ...
            {(() => {
              ...
              return <p className="text-[11px] leading-tight font-medium opacity-90">{reason}</p>;
            })()}
          </div>
          ...
          <div className="mt-auto grid grid-cols-2 gap-x-4 gap-y-3 pt-4 border-t border-black/5 dark:border-white/5">
            <div className="flex flex-col">
              <span className="text-[8px] text-muted-foreground uppercase font-bold truncate opacity-60">
                {leadLabel}
              </span>
            ...
            <div className="flex flex-col items-end">
              <span className="text-[8px] text-muted-foreground uppercase font-bold opacity-60">
                Ticket Médio
              </span>
