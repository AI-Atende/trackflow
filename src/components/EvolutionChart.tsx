...
interface EvolutionChartProps {
  data: EvolutionDataPoint[] | { data: EvolutionDataPoint[]; journeyMap?: string[] };
  loading?: boolean;
}
...
export const EvolutionChart: React.FC<EvolutionChartProps> = ({ data, loading }) => {
  ...
  const defaultLabels = ['Leads', 'Agendados', 'Comparecidos', 'Vendas'];
  const rawData: EvolutionDataPoint[] = Array.isArray(data) ? data : data?.data || [];
  const journeyLabels: string[] = Array.isArray(data)
    ? defaultLabels
    : data.journeyMap || defaultLabels;
      ...
      <div className="flex items-center justify-between mb-6 shrink-0">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Acompanhe as etapas da sua jornada e receita ao longo do tempo.
          </p>
        </div>
