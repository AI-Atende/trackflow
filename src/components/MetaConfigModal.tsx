...
import { MetaAdAccountOption } from '@/lib/meta/types';
...
export const MetaConfigModal: React.FC<MetaConfigModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const [availableAccounts, setAvailableAccounts] = useState<MetaAdAccountOption[]>([]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Conecte sua conta e defina as métricas.
              </p>
            </div>
          </div>
          <button
              ...
              <div className="bg-secondary/10 p-4 rounded-xl border border-border space-y-4">
                ...
                {!isConnected ? (
                  <div className="flex flex-col gap-3">
                    <p className="text-sm text-muted-foreground">
                      Conecte sua conta do Facebook para importar seus anúncios.
                    </p>
                    <button
                        ...
                        <div className="flex justify-between items-center pt-2">
                          ...
                          <button
                            onClick={async () => {
                              if (
                                !confirm(
                                  'Tem certeza que deseja desconectar? Isso irá parar a sincronização.',
                                )
                              )
                                return;
                              try {
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                          <p className="text-sm text-foreground font-medium">
                            1. Selecione o Portfólio Empresarial:
                          </p>
                          <Select
                            options={[
                              ...Array.from(
                                new Set(availableAccounts.map((a) => a.business?.id || 'personal')),
                              ).map((bizId) => {
                                const account = availableAccounts.find(
                                  (a) => (a.business?.id || 'personal') === bizId,
                                );
                                const isPersonal = bizId === 'personal';
                                return {
                                  value: bizId,
                                  label: isPersonal
                                    ? 'Conta Pessoal (Sem Portfólio)'
                                    : account?.business?.name || 'Portfólio Desconhecido',
                                };
                              }),
                            ]}
                            ...
                            <div className="flex gap-2">
                              <Select
                                options={availableAccounts
                                  .filter(
                                    (a) => (a.business?.id || 'personal') === selectedBusinessId,
                                  )
                                  .map((a) => ({
