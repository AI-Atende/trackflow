  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        const isPasswordValid = await bcrypt.compare(credentials.password, client.passwordHash);
        ...
        const primaryAdAccount =
          client.metaAdAccounts?.find((a) => a.status === 'ACTIVE') || client.metaAdAccounts?.[0];
        ...
        return {
          ...
          address: client.address as Address | null,
      ...
      if (token.clientId) {
        ...
        if (dbUser) {
          ...
          const primaryMetaAccount =
            dbUser.metaAdAccounts.find((a) => a.status === 'ACTIVE') || dbUser.metaAdAccounts[0];
          ...
          token.address = dbUser.address as Address | null;
